...
class ROMHeader(object):
    ...
    def __does_header_match(self, rom_byte_array, start_addr):
        ...
        match_complement = (
            int(checksum, 16) + int(checksum_complement, 16) == 65535
        )
    ...
    def parse_header(self, rom_byte_array, start_address):
        ...
        self.licence_code = get_two_bytes_little_endian(
            rom_byte_array[addr + 25], rom_byte_array[addr + 26]
        )
        self.version = rom_byte_array[addr + 27]
        self.checksum_complement = get_two_bytes_little_endian(
            rom_byte_array[addr + 28], rom_byte_array[addr + 29]
        )
        self.checksum = get_two_bytes_little_endian(
            rom_byte_array[addr + 30], rom_byte_array[addr + 31]
        )

    # Find mem addr of interrupt code
    def parse_header_interrups(self, rom_byte_array, start_address):
        addr = start_address
        self.native_cop_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 38], rom_byte_array[addr + 39]
        )
        self.native_brk_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 40], rom_byte_array[addr + 41]
        )
        self.native_abort_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 42], rom_byte_array[addr + 43]
        )
        self.native_reset_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 44], rom_byte_array[addr + 45]
        )
        self.native_irq_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 46], rom_byte_array[addr + 47]
        )
        # co processor enable
        self.cop_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 54], rom_byte_array[addr + 55]
        )
        self.abort_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 56], rom_byte_array[addr + 57]
        )
        self.nmi_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 58], rom_byte_array[addr + 59]
        )
        # execution begins at reset code (entry point of game)
        self.reset_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 60], rom_byte_array[addr + 61]
        )
        self.irq_int_addr = get_two_bytes_little_endian(
            rom_byte_array[addr + 62], rom_byte_array[addr + 63]
        )   # TODO BRK
    ...
    def dump(self):
        ...
        info = (
            ' (' + ROMHeader.rom_type_map.get(self.rom_type, 'UNKNOWN') + ')'
        )
        print('ROM TYPE:           \t' + hex(self.rom_type) + info)
        info = (
            ' (' + ROMHeader.rom_size_map.get(self.rom_size, 'UNKNOWN') + ')'
        )
        ...
        print(
            'CHECKSUM COMPLEMENT:\t0x'
            + hex(self.checksum_complement)
            + ' (NOT CHECKSUM)'
        )
        print(
            'CHECKSUM:           \t0x'
            + hex(self.checksum)
            + ' (SUM OF BYTES AND 0xFFFF)'
        )
        print(
            'CHECKSUM STATUS:    \t'
            + str(hex(self.checksum) == self.expected_checksum)
        )
        print('')
        print(
            'NATIVE COP INTERRUPT ADDR:  \t0x' + hex(self.native_cop_int_addr)
        )
        print(
            'NATIVE BRK INTERRUPT ADDR:  \t0x' + hex(self.native_brk_int_addr)
        )
        print(
            'NATIVE ABORT INTERRUPT ADDR:\t0x'
            + hex(self.native_abort_int_addr)
        )
        print(
            'NATIVE RESET INTERRUPT ADDR:\t0x'
            + hex(self.native_reset_int_addr)
        )
        print(
            'NATIVE IRQ INTERRUPT ADDR:  \t0x' + hex(self.native_irq_int_addr)
        )
